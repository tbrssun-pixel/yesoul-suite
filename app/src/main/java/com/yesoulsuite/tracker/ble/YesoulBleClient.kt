...
class YesoulBleClient(private val context: Context) {
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        val merged = (_devices.value.filterNot { it.address == item.address } + item)
            .sortedWith(compareByDescending<BleDeviceItem> { likelyFitnessDevice(it) }.thenByDescending { it.rssi })
        _devices.value = merged
    }

    private fun likelyFitnessDevice(item: BleDeviceItem): Boolean {
        val name = item.name?.lowercase(Locale.US).orEmpty()
        return "yesoul" in name ||
            "bike" in name ||
            item.serviceUuids.any {
                it.equals(BleUuids.FITNESS_MACHINE_SERVICE.toString(), ignoreCase = true) ||
                    it.equals(BleUuids.CYCLING_SPEED_AND_CADENCE_SERVICE.toString(), ignoreCase = true)
            }
    }
